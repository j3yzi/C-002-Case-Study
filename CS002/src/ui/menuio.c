...
// Menu definitions
Menu mainMenu = {1, "CS002 - Employee Management System", (MenuOption[]){
    {'1', "Employee Management", false, false, 9, 0, 7, 0, 8, 0, NULL},
    {'2', "Load Employee Data", false, false, 9, 0, 7, 0, 8, 0, NULL},
    {'3', "Save Employee Data", true, false, 9, 0, 7, 0, 8, 0, NULL},
    {'4', "Payroll Report", true, false, 9, 0, 7, 0, 8, 0, NULL},
    {'5', "Exit", false, false, 9, 0, 7, 0, 8, 0, NULL}}, 5};
    
Menu employeeSubMenu = {2, "Employee Management", (MenuOption[]){
...
void checkStates(void) {
    ...
    if (employeeListCreated.isEnabled) {
        // Enable "Add Employee" option
        employeeSubMenu.options[1].isDisabled = false;
        
        // Check if list has employees for options that require data
        if (employeeList != NULL && employeeList->head != NULL && employeeList->size > 0) {
            employeeSubMenu.options[2].isDisabled = false; // Edit Employee
            employeeSubMenu.options[3].isDisabled = false; // Delete Employee
            employeeSubMenu.options[4].isDisabled = false; // Search Employee
            employeeSubMenu.options[5].isDisabled = false; // Display All Employees
            mainMenu.options[2].isDisabled = false; // Save Employee Data
            mainMenu.options[3].isDisabled = false; // Payroll Report
        } else {
            // List exists but is empty
            employeeSubMenu.options[2].isDisabled = true;
            employeeSubMenu.options[3].isDisabled = true;
            employeeSubMenu.options[4].isDisabled = true;
            employeeSubMenu.options[5].isDisabled = true;
            mainMenu.options[2].isDisabled = true;
            mainMenu.options[3].isDisabled = true;
        }
        
        // Disable "Create Employee List" since it already exists
        employeeSubMenu.options[0].isDisabled = true;
    } else {
        // Employee list not created yet
        mainMenu.options[2].isDisabled = true;
        mainMenu.options[3].isDisabled = true;
        
        employeeSubMenu.options[0].isDisabled = false; // Enable "Create Employee List"
        employeeSubMenu.options[1].isDisabled = true;  // Disable "Add Employee"
        employeeSubMenu.options[2].isDisabled = true;
        employeeSubMenu.options[3].isDisabled = true;
        employeeSubMenu.options[4].isDisabled = true;
        employeeSubMenu.options[5].isDisabled = true;
    ...
    do {
        ...
        switch(mainMenuChoice) {
            ...
            case '4':
                ...
                break;
            case '5':
                printf("\nExiting CS002 Employee Management System...\n");
                return 0;
            default:
...
static void handleEmployeeManagement(void) {
    ...
    do {
        checkStates();
        winTermClearScreen();
        choice = initMenu(&employeeSubMenu);
        
        switch(choice) {
            case '1':
                if (!employeeSubMenu.options[0].isDisabled) {
                ...
                break;
            case '2':
                if (!employeeSubMenu.options[1].isDisabled) {
                    handleAddEmployee();
                } else {
                ...
                break;
            case '3':
                if (!employeeSubMenu.options[2].isDisabled) {
                ...
                break;
            case '4':
                if (!employeeSubMenu.options[3].isDisabled) {
                ...
                break;
            case '5':
                if (!employeeSubMenu.options[4].isDisabled) {
                ...
                break;
            case '6':
                if (!employeeSubMenu.options[5].isDisabled) {
                    winTermClearScreen();
...
static int handleCreateEmployeeList(void) {
    ...
    if (employeesCreated > 0) {
        ...
        printf("You can add employees using the 'Add Employee' option later.\n");
